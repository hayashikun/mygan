...
def make_generator(noise_dim):
    ...
    model.add(layers.Dense(4 * 4 * 2048, use_bias=False, input_shape=(noise_dim,)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    model.add(layers.Reshape((4, 4, 2048)))
    assert model.output_shape == (None, 4, 4, 2048)

    model.add(layers.Conv2DTranspose(1024, kernel_size=5, strides=2, padding="same", use_bias=False))
...
def make_discriminator():
    ...
    model.add(layers.Conv2D(128, kernel_size=5, strides=2, padding="same", input_shape=(128, 128, 3)))
    model.add(layers.LeakyReLU())
    ...
    model.add(layers.Conv2D(2048, kernel_size=5, strides=2, padding="same"))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))
    assert model.output_shape == (None, 4, 4, 2048)
